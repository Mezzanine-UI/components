...
export interface TodoItemFieldProps<T = any> {
  registerName: string;
  width?: number;
  maxLength?: number;
  minLength?: number;
...
export type TodoListFieldProps<
  T extends Omit<
    TodoItemFieldProps,
    'controller' | 'onUpdateContext' | 'onRemove'
  > = Omit<TodoItemFieldProps, 'controller' | 'onUpdateContext' | 'onRemove'>,
> = HookFormFieldProps<
  FieldValues,
  T,
  {
...
export const TodoListField: HookFormFieldComponent<TodoListFieldProps> = ({
  autoComplete,
  autoFocus,
  className,
  listClassName,
  ...
  const todoItems: TodoItem[] = useMemo(() => controller.todoItems, [itemSize]);
  ...
  const TodoComponent = useMemo(
    () => (
      <ol ref={oListRef} className={cx(classes.ol, listClassName)}>
        {todoItems.map((todo) => (
          <BaseField
            key={todo.id}
